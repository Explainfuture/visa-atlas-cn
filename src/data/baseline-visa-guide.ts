...
const specialSources: Partial<Record<string, VisaGuide["sources"]>> = {
  ...
  tw: [
    {
      title: "大陆地区人民申请进入台湾地区送件须知",
      authority: "台湾地区内政事务主管部门移民机构",
...
function createSpecialDocumentGuide(country: CountrySummary, entry: VisaBaselineEntry): VisaGuide | undefined {
  ...
  if (country.code === "tw") {
    return {
      ...
      overview: "大陆居民赴台湾地区不是普通护照电子签流程。能否以旅游目的申请，取决于当前开放对象、户籍/居住地、出发地与团体或个人渠道；确认有资格后，仍要分别办妥大陆端有效证件和入台许可。",
      decision: "第一步不是买机票，而是向大陆端出入境主管部门和台湾地区移民主管机构同时确认：你的身份、居住地与出发地目前是否有可用的旅游申请渠道。",
      cost: {
        summary: "大陆端证件费 + 入台许可费",
        items: [
          { label: "大陆端旅行证件 / 签注", amount: "以当前受理项目为准", detail: "只在确认本人符合开放范围后，按国家移民管理机构的收费单支付。" },
          { label: "入台许可", amount: "以台湾地区官方申请页为准", detail: "类别、递交地与申请方式不同，收费会不同；商业代办费另计。" },
          { label: "旅行社与材料服务", amount: "按实际渠道", detail: "若政策限定团队或指定渠道，问清是否包含许可、保险、翻译和取消费用。" },
        ],
        note: "渠道未开放或资格未确认前，不要向代办支付不可退许可费或团费。",
      },
      steps: [
        { title: "先做双向资格核验", detail: "分别确认大陆端是否受理、台湾地区是否接受该类入台申请；说明户籍、常住地、出发地、旅行目的与日期。", action: { label: "查看入台申请须知", url: "https://www.immigration.gov.tw/5382/5385/7244/7250/7257/%E5%81%9C%E7%95%99/" } },
